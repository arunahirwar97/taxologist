...
data "template_file" "api_container_definitions" {
  template = file("./templates/ecs/container-definations.json.tpl")

  vars = {
    app_image                = var.ecr_image_api
    proxy_image              = var.ecr_image_proxy
    django_secret_key        = var.django_secret_key
    paytm_merchant_id        = var.paytm_merchant_id
    paytm_secret_key         = var.paytm_secret_key
    ...
    allowed_hosts            = aws_route53_record.mytaxboard.fqdn
    s3_storage_bucket_name   = aws_s3_bucket.app_private_files.bucket
...
resource "aws_security_group" "ecs_service" {
  ...
  egress {
    ...
    #   "10.1.10.0/24",
    #   "10.1.11.0/24",
    # ]    # cidr_blocks=[ module.vpc.subnets["public"].cidr_block
    cidr_blocks = [
...
data "template_file" "ecs_s3_write_policy" {
  ...
  vars = {
    bucket_arn = aws_s3_bucket.app_private_files.arn
  }
}
